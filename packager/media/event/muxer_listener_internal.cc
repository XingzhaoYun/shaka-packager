...
void AddAudioInfo(const AudioStreamInfo* audio_stream_info,
  ...
  if ((audio_stream_info->codec() == kCodecAC4)) {
    ...
    if (!CalculateAC4ChannelConfigMpegValue(codec_config,
                                            ac4_channel_mpeg_value)) {
      LOG(ERROR) << "Failed to calculate AC4 channel config corresponding"
                    " mpeg value.";
      return;
    }
    audio_info->mutable_codec_specific_data()->
        set_ac4_channel_config_mpeg_value(ac4_channel_mpeg_value);
    ...
    audio_info->mutable_codec_specific_data()->set_ac4_is_ims(
        ac4_payload & 0x01);
    audio_info->mutable_codec_specific_data()->set_ac4_is_atmos(
        (ac4_payload >> 1) & 0x01);
  }
}
